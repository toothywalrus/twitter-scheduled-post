...
import twitter
from timedelta import fields

from djcelery.models import PeriodicTask, IntervalSchedule
from datetime import datetime
from .tasks import tweet, start_tweet_set, post_next_tweet
...
class TaskScheduler(PeriodicTask):
	...
	def create(cls, task_name, period, every, args="[]", kwargs="{}"):
		...
		ts = cls(name=per_task_name, task=task_name, interval=interval, args=args, kwargs=kwargs, enabled=False)
		return ts
...
class Tweet(models.Model):
	...
	username = models.CharField(max_length=140, default='vladboyyko', editable=False)
...
class PostTweetSet(models.Model):
	...
	def next_tweet(self):
		try:
			return self.tweets.filter(tweet__already_posted=False).order_by('priority')[0]
		except Tweet.DoesNotExist:
			pass

	def save(self, *args, **kwargs):
		super(PostTweetSet, self).save(*args, **kwargs)
		start_tweet_set.apply_async(args=[self.pk, self.interval.period, self.interval.every], eta=self.start_time)

	def __unicode__(self):
		return "%s, %s, start time: %s" % (self.description, self.interval, self.start_time)
